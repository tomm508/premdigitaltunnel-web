...
import { db, doc, collection, onSnapshot, setDoc, updateDoc, getDoc, getDocs } from '../lib/firebase';
...
export const AdminPanel: React.FC<AdminPanelProps> = ({ isDark, userRole }) => {
  ...
  const [adminCountdown, setAdminCountdown] = useState<{ days: number; hours: number; minutes: number; seconds: number }>({
    days: 0,
    hours: 0,
    minutes: 0,
    seconds: 0
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, 'platform', 'settings'), (docSnap) => {
      if (docSnap.exists()) {
        ...
        setResetDays(data.resetDays ? Number(data.resetDays) : 1);
  ...
  // Real-time admin countdown calculator according to resetDays & resetTime
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      if (daysInterval === 1) {
        // Daily cycle: target today or tomorrow at targetHour:targetMinute
        ...
        setAdminCountdown({ days: 0, hours, minutes, seconds });
      } else {
        ...
        setAdminCountdown({ days, hours, minutes, seconds });
      }
    }, 1000);
  ...
  // Instant Reset Quota for All Servers
  const handleResetAllQuota = async () => {
    if (!window.confirm("Yakin ingin mereset kuota terpakai (usedSlots) semua server gratis menjadi 0 sekarang?")) return;
    setIsResettingQuota(true);
    try {
      const snap = await getDocs(collection(db, 'vps_nodes'));
      const batchPromises = snap.docs.map(d => 
        setDoc(doc(db, 'vps_nodes', d.id), { usedSlots: 0, lastResetAt: new Date().toISOString() }, { merge: true })
      );
      await Promise.all(batchPromises);
      alert("Berhasil! Kuota semua server gratis telah direset ke 0.");
    } catch (err: any) {
      console.error(err);
            ...
            {activeTab === 'settings' && (
              <div>
                ...
                <form onSubmit={handleSaveSettings} className="space-y-8">
                  ...
                  {/* Free Account Rules & Reset Schedule */}
                  <div className="space-y-5">
                    <div className="flex items-center justify-between">
                      <h3 className="text-sm font-bold text-emerald-400 uppercase tracking-wider flex items-center gap-2">
                        <Clock className="w-4 h-4 text-emerald-400" />
                        Free Account Policies & Reset Schedule
                      </h3>
                      <span className="text-[11px] font-semibold px-2.5 py-0.5 rounded-full bg-emerald-950 border border-emerald-500/30 text-emerald-300">
                        {resetDays === 1 ? '1 Day (Per Hari)' : `${resetDays} Days Cycle`}
                      </span>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs text-slate-400 mb-1.5 font-medium">
                          Free Account Limit (Total per server)
                        </label>
                        <input
                          type="number"
                          value={freeLimit}
                        ...
                        />
                        <span className="text-[11px] text-slate-500 mt-1 block">Batas maksimal kuota akun gratis per node server</span>
                      </div>
                      <div>
                        <label className="block text-xs text-slate-400 mb-1.5 font-medium">
                          Free Account Reset Time (Jam Eksekusi)
                        </label>
                        <div className="relative">
                          ...
                          <input
                            ...
                            className="w-full bg-[#13172a] border border-slate-600 rounded-xl pl-11 pr-4 py-2.5 text-white focus:outline-none focus:border-indigo-500"
                          />
                        </div>
                        <span className="text-[11px] text-slate-500 mt-1 block">Waktu eksekusi reset kuota otomatis (WIB)</span>
                      </div>
                    </div>

                    {/* Cycle Days Selection: 1 Day, 2 Days, 3 Days */}
                    <div>
                      <label className="block text-xs text-slate-300 font-semibold mb-2">
                        Pilihan Siklus Countdown Reset Kuota (Hari):
                      </label>
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        {/* 1 Day Option */}
                        <button
                          type="button"
                          onClick={() => setResetDays(1)}
                          className={`p-3.5 rounded-2xl border text-left transition-all relative ${
                            resetDays === 1
                              ? 'bg-emerald-950/40 border-emerald-400 text-white shadow-lg shadow-emerald-900/20'
                              : 'bg-[#13172a] border-slate-700 text-slate-400 hover:border-slate-500 hover:text-slate-200'
                          }`}
                        >
                          <div className="flex items-center justify-between mb-1.5">
                            <span className="font-bold text-sm text-emerald-300">1 Day (24 Jam)</span>
                            {resetDays === 1 && (
                              <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />
                            )}
                          </div>
                          <p className="text-[11px] leading-relaxed text-slate-300">
                            Hitungan <strong>per hari</strong>: 23:59:59 menuju 00:00:00 setiap 24 jam.
                          </p>
                        </button>
                        ...
                        >
                          <div className="flex items-center justify-between mb-1.5">
                            <span className="font-bold text-sm text-emerald-300">2 Days (48 Jam)</span>
                            {resetDays === 2 && (
                              <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />
                            )}
                          </div>
                          <p className="text-[11px] leading-relaxed text-slate-300">
                            Hitungan <strong>2 hari sekali</strong>: siklus countdown 48 jam penuh.
                          </p>
                        </button>
                        ...
                        >
                          <div className="flex items-center justify-between mb-1.5">
                            <span className="font-bold text-sm text-emerald-300">3 Days (72 Jam)</span>
                            {resetDays === 3 && (
                              <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />
                            )}
                          </div>
                          <p className="text-[11px] leading-relaxed text-slate-300">
                            Hitungan <strong>3 hari sekali</strong>: siklus countdown 72 jam penuh.
                          </p>
                        </button>
                      </div>
                    </div>

                    {/* Live Countdown Display Card in Admin Panel */}
                    <div className="bg-[#0e1222] border border-emerald-500/30 rounded-2xl p-4 sm:p-5 flex flex-col sm:flex-row items-center justify-between gap-4">
                      <div className="space-y-1 text-center sm:text-left">
                        <div className="flex items-center justify-center sm:justify-start gap-2">
                          ...
                          <span className="text-[10px] px-2 py-0.5 rounded-md bg-emerald-950/80 border border-emerald-500/30 text-emerald-300 font-semibold">
                            {resetDays === 1 ? '1 Day (23:59 - 00:00 Perhari)' : `${resetDays} Days (${resetDays * 24} Jam)`}
                          </span>
                        </div>
                        <p className="text-xs text-slate-400">
                          {resetDays === 1
                            ? `Siklus per hari (23:59:59 → 00:00:00) • Eksekusi reset setiap pukul ${resetTime} WIB`
                            : `Siklus reset ${resetDays} hari sekali (${resetDays * 24} jam) • Eksekusi pukul ${resetTime} WIB`}
                        </p>
                      </div>

                      <div className="text-center sm:text-right bg-[#13182c] border border-slate-700/60 rounded-xl px-5 py-3 shrink-0">
                        <div className="font-mono text-2xl sm:text-3xl font-black text-emerald-300 tracking-wider">
                          {resetDays > 1 && `${adminCountdown.days}d `}
                          {adminCountdown.hours.toString().padStart(2, '0')}:{adminCountdown.minutes.toString().padStart(2, '0')}:{adminCountdown.seconds.toString().padStart(2, '0')}
                        </div>
                        <span className="text-[10px] text-slate-400 font-medium">Sisa waktu menuju reset otomatis</span>
                      </div>
                    </div>

                    {/* Manual Instant Reset Quota Action */}
                    <div className="pt-2 flex flex-col sm:flex-row items-center justify-between gap-3 border-t border-slate-800">
                      <p className="text-xs text-slate-400">
                        Ingin mengosongkan kuota server sekarang tanpa menunggu jam reset?
                      </p>
                      <button
                        type="button"
                        onClick={handleResetAllQuota}
                        disabled={isResettingQuota}
                        className="px-4 py-2 rounded-xl text-xs font-bold text-amber-300 bg-amber-950/50 hover:bg-amber-900/60 border border-amber-500/40 transition-all flex items-center gap-2 whitespace-nowrap"
                      >
                        <RefreshCw className={`w-3.5 h-3.5 ${isResettingQuota ? 'animate-spin' : ''}`} />
                        <span>{isResettingQuota ? 'Mereset Semua Kuota...' : 'Reset Semua Kuota ke 0 Sekarang'}</span>
                      </button>
                    </div>
                  </div>
